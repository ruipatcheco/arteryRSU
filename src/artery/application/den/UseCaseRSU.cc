...
void UseCaseRSU::initialize(int stage)
{
    if (stage == 0) {
...
vanetza::asn1::Denm UseCaseRSU::createMessageSkeleton()
{
    ...
    const auto taiTime = 0;
            //countTaiMilliseconds(mService->getTimer()->getTimeFor(mVdp->updated()));
    ...
    message->denm.management.eventPosition.longitude = 0;
            //round(mVdp->longitude(), microdegree) * Longitude_oneMicrodegreeEast;

    message->denm.management.eventPosition.latitude = 0;

            //round(mVdp->latitude(), microdegree) * Latitude_oneMicrodegreeNorth;
    ...
    message->denm.location->eventSpeed->speedValue = 0;
        //= std::abs(round(mVdp->speed(), centimeter_per_second)) * SpeedValue_oneCentimeterPerSec;
